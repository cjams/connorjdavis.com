...
<FunctionVisualizer 
  ...
  domain={{"x": [-1.75, 1.75], "y": [-1.75, 1.75]}} 
  zRange={[-2, 2]}
  resolution={50}
  colorScheme="viridis"
  height="500px"
...
In addition to graphing the values of the function as we did above, we can also create 
a graph that represents the data flow through the function and its operands. This
is called an _expression graph_ or _computational graph_. Below is the expression graph for ```f(2, 3)```:

![expression graph](/static/media/expression-graph.svg)

Notice the ```x``` and ```y``` ```data``` values at the top are initialized with ```2``` and ```3``` respectively.
You can trace these input values as they flow downward through the function's intermediate expressions
(denoted by they symbol ```**2```, i.e. the square of the inputs) until the final expression ```+```, giving a final
value of ```f(2, 3) == 13```. This flow of inputs through each expression to the final output is called the 
_**forward pass**_. So a forward pass is just another name for the evaluation of the function given some inputs. The process
of doing multiple forward passes with different inputs is called _**inference**_.

Now you are probably wondering what the ```grad``` values are for? The ```grad``` stands for the _**gradient**_.
The gradient is a concept from calculus which captures the influence that a particular variable has on the 
final value of the function. The gradient at each node is calculated via the [Chain Rule](https://en.wikipedia.org/wiki/Chain_rule). 
However instead of starting from the top (inputs), you start from the bottom (output).
to ```f``` is 1.
